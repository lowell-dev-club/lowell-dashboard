...
def send_reset_email(user):
    ...
    msg = Message('Password Reset Request', sender='LowellHelpForum@gmail.com', recipients=[user.email])
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        hashed_password = bcrypt.generate_password_hash(sha256((form.password.data + user.email + SECRET_SALT).encode()).hexdigest()).decode('utf-8')
